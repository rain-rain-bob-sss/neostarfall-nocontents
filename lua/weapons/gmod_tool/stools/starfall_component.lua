...
if SERVER then
	...
	function MakeComponent(class, pl, Pos, Ang, model)
		if not pl:CheckLimit("starfall_components") then
			return false
		end
	...
	language.Add(
		"Tool.starfall_component.desc",
		"Spawns a Neostarfall component. (Press Shift+F to switch to the processor tool)"
	)
...
function TOOL:GetPos(ent, trace, model, disable_flat)
	...
	elseif
		self.ClientConVar.createflat
		and (createflat == 1) ~= ((string.find(model, "pcb") or string.find(model, "hunter")) ~= nil)
	then
...
function TOOL:RightClick(trace)
	if not trace.HitPos or not (trace.Entity and trace.Entity:IsValid()) or trace.Entity:IsPlayer() then
		return false
	end
	if CLIENT then
		return true
	...
	elseif self:GetStage() == 1 then -- stage 1: right-clicking on something links it
		if not (self.Component and self.Component:IsValid()) then
			self:SetStage(0)
			return
		end
...
if CLIENT then
	...
	function TOOL.BuildCPanel(panel)
		panel:AddControl(
			"Header",
			{ Text = "#Tool.starfall_component.name", Description = "#Tool.starfall_component.desc" }
		)
		panel:AddControl(
			"CheckBox",
			{ Label = "#Tool.starfall_component.parent", Command = "starfall_component_parent" }
		)
		panel:AddControl(
			"CheckBox",
			{ Label = "#Tool.starfall_component.createflat", Command = "starfall_component_createflat" }
		)

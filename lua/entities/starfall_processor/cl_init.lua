...
function ENT:GetOverlayText()
	...
	if ent_tbl.instance then
		...
		clientstr = tostring(math.Round(bufferAvg * 1000000))
			.. "us. ("
			.. tostring(math.floor(bufferAvg / ent_tbl.instance.cpuQuota * 100))
			.. "%)"
...
function ENT:DrawCustomOverlay()
	...
	if ent_tbl.OverlayFade > 0 then
		...
		render.DrawQuad(
			Vector(pos.x - 128, pos.y - 300, 0),
			Vector(pos.x + 128, pos.y - 300, 0),
			Vector(pos.x + 128, pos.y - 44, 0),
			Vector(pos.x - 128, pos.y - 44, 0),
			Color(255, 255, 255, ent_tbl.OverlayFade * 255)
		)
	end
end
...
function ENT:SetReuploadOnReload(enabled)
	if enabled and not self.reuploadOnReload then
		hook.Add("StarfallEditorFileReload", self, function(_, reloaded_file)
			if not self:DependsOnFile(reloaded_file) then
				return
			end
...
hook.Add(
	"StarfallError",
	"StarfallErrorReport",
	function(_, owner, client, main_file, message, traceback, should_notify)
		if not Ent_IsValid(owner) then
			return
		end
		...
		if owner == local_player then
			...
			elseif client then
				if should_notify then
					SF.AddNotify(
						owner,
						string.format("Neostarfall '%s' errored for player %s", main_file, client:Nick()),
						"ERROR",
						7,
						"SILENT"
					)
					...
					print(
						string.format("Neostarfall '%s' errored for player %s: %s", main_file, client:Nick(), message)
					)
				end
			end
...
net.Receive("starfall_processor_download", function(len)
	net.ReadStarfall(nil, function(ok, sfdata, err)
		...
		elseif Ent_IsValid(sfdata.proc) and Ent_IsValid(sfdata.owner) then
			...
			sfdata.proc:Error({
				message = "Failed to download and initialize client: " .. tostring(err),
				traceback = "",
			})
		end
	end)
...
net.Receive("starfall_processor_used", function(len)
	...
	if not Ent_IsValid(chip) then
		return
	end
	if not Ent_IsValid(used) then
		return
	end
	...
	if
		activator == LocalPlayer()
		and instance.player ~= SF.Superuser
		and instance.permissionRequest
		and instance.permissionRequest.showOnUse
...
SF.BlockedUsers = SF.BlockedList("user", "running clientside neostarfall code", "sf_blockedusers.txt", function(steamid)

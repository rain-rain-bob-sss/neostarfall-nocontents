...
local plyPropBurst = SF.BurstObject(
	"props",
	"props",
	4,
	4,
	"Rate props can be spawned per second.",
	"Number of props that can be spawned in a short time."
)
...
local plyVertexCount = SF.LimitObject(
	"props_custom_vertices",
	"custom prop vertices",
	14400,
...
return function(instance)
	...
	function props_library.create(pos, ang, model, frozen)
		...
		if ply ~= SF.Superuser and gamemode.Call("PlayerSpawnProp", ply, model) == false then
			SF.Throw("Another hook prevented the prop from spawning", 2)
		end
		...
		if not propent:GetModel() then
			propent:Remove()
			SF.Throw("Invalid model", 2)
		end
	...
	function props_library.createRagdoll(model, frozen)
		...
		plyPropBurst:use(ply, 1)
		entList:checkuse(ply, 1)
		if ply ~= SF.Superuser and gamemode.Call("PlayerSpawnRagdoll", ply, model) == false then
			SF.Throw("Another hook prevented the ragdoll from spawning", 2)
		end
	...
	function props_library.createCustom(pos, ang, vertices, frozen)
		...
		plyPropBurst:use(ply, 1)
		entList:checkuse(ply, 1)
		if instance.player ~= SF.Superuser and gamemode.Call("PlayerSpawnProp", ply, "starfall_prop") == false then
			SF.Throw("Another hook prevented the prop from spawning", 2)
		end
		...
		for k, v in ipairs(vertices) do
			if k > maxConvexesPerProp then
				SF.Throw("Exceeded the max convexes per prop (" .. maxConvexesPerProp .. ")", 2)
			end
			...
			for o, p in ipairs(v) do
				if o > maxVerticesPerConvex then
					SF.Throw("Exceeded the max vertices per convex (" .. maxVerticesPerConvex .. ")", 2)
				end
				local vec = vunwrap(p)
				if math.abs(vec.x) > max or math.abs(vec.y) > max or math.abs(vec.z) > max then
					SF.Throw("The custom prop cannot exceed a hull size of " .. max, 2)
				end
				if vec.x ~= vec.x or vec.y ~= vec.y or vec.z ~= vec.z then
					SF.Throw("Your mesh contains nan values!", 2)
				end
				for i = 1, o - 1 do
	...
	function props_library.createComponent(pos, ang, class, model, frozen)
		...
		if not allowed_components[class] then
			return SF.Throw("Invalid class!", 1)
		end
		...
		if ply ~= SF.Superuser then
			if not ply:CheckLimit("starfall_components") then
				SF.Throw("Limit of components reached!", 2)
			end
			plyPropBurst:use(ply, 1)
			entList:checkuse(ply, 1)
			if gamemode.Call("PlayerSpawnSENT", ply, class) == false then
				SF.Throw("Another hook prevented the component from spawning", 2)
			end
		end
	...
	function props_library.createSent(pos, ang, class, frozen, data)
		...
		if swep then
			if ply ~= SF.Superuser then
				if (not swep.Spawnable and not ply:IsAdmin()) or (swep.AdminOnly and not ply:IsAdmin()) then
					SF.Throw("This swep is admin only!", 2)
				end
				if gamemode.Call("PlayerSpawnSWEP", ply, class, swep) == false then
					SF.Throw("Another hook prevented the swep from spawning", 2)
				end
			end
		...
		elseif sent then
			if ply ~= SF.Superuser then
				if sent.AdminOnly and not ply:IsAdmin() then
					SF.Throw("This sent is admin only!", 2)
				end
				if gamemode.Call("PlayerSpawnSENT", ply, class) == false then
					SF.Throw("Another hook prevented the sent from spawning", 2)
				end
			end
		...
		elseif npc then
			if ply ~= SF.Superuser then
				if npc.AdminOnly and not ply:IsAdmin() then
					SF.Throw("This npc is admin only!", 2)
				end
				if gamemode.Call("PlayerSpawnNPC", ply, class, "") == false then
					SF.Throw("Another hook prevented the npc from spawning", 2)
				end
			end
			...
			if IsValid(entity) then
				...
				if npc.SpawnFlags then
					SpawnFlags = bit.bor(SpawnFlags, npc.SpawnFlags)
				end
				if npc.TotalSpawnFlags then
					SpawnFlags = npc.TotalSpawnFlags
				end
		...
		elseif vehicle then
			if
				ply ~= SF.Superuser
				and gamemode.Call("PlayerSpawnVehicle", ply, vehicle.Model, vehicle.Class, vehicle) == false
			then
				SF.Throw("Another hook prevented the vehicle from spawning", 2)
			end
			...
			if IsValid(entity) then
				...
				if vehicle.KeyValues then
					for k, v in pairs(vehicle.KeyValues) do
						...
						if
							kLower == "vehiclescript"
							or kLower == "limitview"
							or kLower == "vehiclelocked"
							or kLower == "cargovisible"
							or kLower == "enablegun"
						then
							entity:SetKeyValue(k, v)
						end
				...
				if entity.SetVehicleClass then
					entity:SetVehicleClass(class)
				end
		...
		elseif sent2 then
			if ply ~= SF.Superuser then
				if scripted_ents.GetStored(class).t.AdminOnly and not ply:IsAdmin() then
					SF.Throw("This sent is admin only!", 2)
				end
				if gamemode.Call("PlayerSpawnSENT", ply, class) == false then
					SF.Throw("Another hook prevented the sent from spawning", 2)
			...
			for k, v in pairs(data) do
				if not sentparams[k] then
					SF.Throw("Invalid parameter in data: " .. tostring(k), 2)
				end
			end
				...
				if value ~= nil then
					...
					if org[1] == TYPE_COLOR then
						if not IsColor(value) then
							SF.ThrowTypeError("Color", SF.GetType(value), 2, "Parameter: " .. param)
						end
		...
		if IsValid(entity) then
			...
			if CPPI then
				entity:CPPISetOwner(ply == SF.Superuser and NULL or ply)
			end
	...
	function props_library.canSpawn()
		if not SF.Permissions.hasAccess(instance, nil, "prop.create") then
			return false
		end

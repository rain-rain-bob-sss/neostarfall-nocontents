...
registerprivilege(
	"constraints.keepupright",
	"Keepupright",
	"Allows the user to keep an entity upright",
	{ entities = {} }
)
registerprivilege(
	"constraints.ballsocket",
	"Ballsocket",
	"Allows the user to ballsocket two entities",
	{ entities = {} }
)
registerprivilege(
	"constraints.ballsocketadv",
	"BallsocketAdv",
	"Allows the user to advanced ballsocket two entities",
	{ entities = {} }
)
...
local entList =
	SF.EntManager("constraints", "constraints", 600, "The number of constraints allowed to spawn via neostarfall")
...
return function(instance)
	...
	local constr_methods, constr_meta, cwrap, cunwrap =
		instance.Types.Constraint.Methods,
		instance.Types.Constraint,
		instance.Types.Constraint.Wrap,
		instance.Types.Constraint.Unwrap
	...
	local function check_constr_perms(ent)
		if ent.Ent1 and not IsWorld(ent.Ent1) then
			checkpermission(instance, ent.Ent1, "entities.remove", 3)
		end
		if ent.Ent2 and not IsWorld(ent.Ent2) then
			checkpermission(instance, ent.Ent2, "entities.remove", 3)
		end
		if ent.Ent3 and not IsWorld(ent.Ent3) then
			checkpermission(instance, ent.Ent3, "entities.remove", 3)
		end
		if ent.Ent4 and not IsWorld(ent.Ent4) then
			checkpermission(instance, ent.Ent4, "entities.remove", 3)
		end
	end
	...
	-- @return Constraint The constraint entity
	-- @server
	function constraint_library.ballsocketadv(
		e1,
		e2,
		bone1,
		bone2,
		...
		local ent = constraint.AdvBallsocket(
			ent1,
			ent2,
			bone1,
			bone2,
	...
	-- @return Constraint The constraint entity
	-- @server
	function constraint_library.rope(
		index,
		e1,
		e2,
		bone1,

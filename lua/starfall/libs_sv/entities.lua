...
local Ent_AddCallback, Ent_GetTable, Ent_IsScripted, Ent_IsValid, Ent_RemoveCallback =
	ENT_META.AddCallback, ENT_META.GetTable, ENT_META.IsScripted, ENT_META.IsValid, ENT_META.RemoveCallback
local Ply_InVehicle = PLY_META.InVehicle

-- Register privileges
registerprivilege(
	"entities.applyDamage",
	"Apply damage",
	"Allows the user to apply damage to an entity",
	{ entities = {} }
)
registerprivilege(
	"entities.applyForce",
	"Apply force",
	"Allows the user to apply force to an entity",
	{ entities = {} }
)
registerprivilege(
	"entities.setPos",
	"Set Position",
	"Allows the user to teleport an entity to another location",
	{ entities = {} }
)
registerprivilege(
	"entities.setAngles",
	"Set Angles",
	"Allows the user to rotate an entity to another orientation",
	{ entities = {} }
)
registerprivilege(
	"entities.setEyeAngles",
	"Set eye angles",
	"Allows the user to rotate the view of an entity to another orientation",
	{ entities = {} }
)
registerprivilege(
	"entities.setVelocity",
	"Set Velocity",
	"Allows the user to change the velocity of an entity",
	{ entities = {} }
)
registerprivilege(
	"entities.setSolid",
	"Set Solid",
	"Allows the user to change the solidity of an entity",
	{ entities = {} }
)
registerprivilege(
	"entities.setContents",
	"Set Contents",
	"Allows the user to change the contents flag of an entity",
	{ entities = {} }
)
...
registerprivilege(
	"entities.setInertia",
	"Set Inertia",
	"Allows the user to change the inertia of an entity",
	{ entities = {} }
)
registerprivilege(
	"entities.enableGravity",
	"Enable gravity",
	"Allows the user to change whether an entity is affected by gravity",
	{ entities = {} }
)
registerprivilege(
	"entities.enableMotion",
	"Set Motion",
	"Allows the user to disable an entity's motion",
	{ entities = {} }
)
registerprivilege(
	"entities.enableDrag",
	"Set Drag",
	"Allows the user to disable an entity's air resistance and change it's coefficient",
	{ entities = {} }
)
registerprivilege(
	"entities.setDamping",
	"Set Damping",
	"Allows the user to change entity's air friction damping",
	{ entities = {} }
)
...
registerprivilege(
	"entities.canTool",
	"CanTool",
	"Whether or not the user can use the toolgun on the entity",
	{ entities = {} }
)
...
registerprivilege(
	"entities.getTable",
	"GetTable",
	"Allows the user to get an entity's table",
...
local collisionListenerLimit = SF.LimitObject(
	"collisionlistener",
	"collisionlistner",
	128,
	"The number of concurrent neostarfall collision listeners"
)
	...
	__index = {
		create = function(self, ent)
			...
			local function collisionQueueCallback(ent, data)
				...
				queue[nqueue] = data
				if nqueue == 1 then
					timer.Simple(0, collisionQueueProcess)
				end
			end
			...
			SF.CallOnRemove(ent, "RemoveCollisionListeners", function(e)
				self:destroy(e)
			end)
...
return function(instance)
	...
	local Ent_AddCallback, Ent_DrawShadow, Ent_Extinguish, Ent_Fire, Ent_GetChildren, Ent_GetClass, Ent_GetCreationID, Ent_GetForward, Ent_GetFriction, Ent_GetMoveType, Ent_GetParent, Ent_GetPhysicsObject, Ent_GetRight, Ent_GetTable, Ent_GetUp, Ent_GetVar, Ent_Ignite, Ent_IsConstraint, Ent_IsPlayer, Ent_IsPlayerHolding, Ent_IsScripted, Ent_IsValid, Ent_IsVehicle, Ent_IsWorld, Ent_OBBMaxs, Ent_OBBMins, Ent_PhysicsInit, Ent_PhysicsInitSphere, Ent_Remove, Ent_RemoveCallback, Ent_SetAngles, Ent_SetCollisionBounds, Ent_SetCollisionGroup, Ent_SetElasticity, Ent_SetFriction, Ent_SetLightingOriginEntity, Ent_SetLocalAngles, Ent_SetLocalPos, Ent_SetMoveType, Ent_SetNotSolid, Ent_SetPos, Ent_SetSolid, Ent_SetVelocity, Ent_TestPVS, Ent_Use =
		...
		ENT_META.GetVar,
		ENT_META.Ignite,
		ENT_META.IsConstraint,
		ENT_META.IsPlayer,
		ENT_META.IsPlayerHolding,
		ENT_META.IsScripted,
		ENT_META.IsValid,
		ENT_META.IsVehicle,
		ENT_META.IsWorld,
		ENT_META.OBBMaxs,
		ENT_META.OBBMins,
		ENT_META.PhysicsInit,
		ENT_META.PhysicsInitSphere,
		ENT_META.Remove,
		ENT_META.RemoveCallback,
		ENT_META.SetAngles,
		ENT_META.SetCollisionBounds,
		ENT_META.SetCollisionGroup,
		ENT_META.SetElasticity,
		ENT_META.SetFriction,
		ENT_META.SetLightingOriginEntity,
		ENT_META.SetLocalAngles,
		ENT_META.SetLocalPos,
		ENT_META.SetMoveType,
		ENT_META.SetNotSolid,
		ENT_META.SetPos,
		ENT_META.SetSolid,
		ENT_META.SetVelocity,
		ENT_META.TestPVS,
		ENT_META.Use
	...
	local Phys_AddAngleVelocity, Phys_AddVelocity, Phys_ApplyForceCenter, Phys_ApplyForceOffset, Phys_ApplyTorqueCenter, Phys_EnableDrag, Phys_EnableGravity, Phys_EnableMotion, Phys_GetAngleVelocity, Phys_GetMass, Phys_GetMaterial, Phys_IsMoveable, Phys_IsValid, Phys_SetContents, Phys_SetInertia, Phys_SetMass, Phys_Wake =
		PHYS_META.AddAngleVelocity,
		PHYS_META.AddVelocity,
		PHYS_META.ApplyForceCenter,
		PHYS_META.ApplyForceOffset,
		PHYS_META.ApplyTorqueCenter,
		PHYS_META.EnableDrag,
		PHYS_META.EnableGravity,
		PHYS_META.EnableMotion,
		PHYS_META.GetAngleVelocity,
		PHYS_META.GetMass,
		PHYS_META.GetMaterial,
		PHYS_META.IsMoveable,
		PHYS_META.IsValid,
		PHYS_META.SetContents,
		PHYS_META.SetInertia,
		PHYS_META.SetMass,
		PHYS_META.Wake
	...
	function ents_methods:linkComponent(e)
		...
		if e then
			...
			if
				Ent_GetClass(link) == "starfall_processor"
				and (Ent_GetClass(ent) == "starfall_screen" or Ent_GetClass(ent) == "starfall_hud")
			then
				SF.LinkEnt(ent, link)
	...
	function ents_methods:setCustomPropForces(ang, lin, mode)
		local ent = getent(self)
		local ent_tbl = Ent_GetTable(ent)
		if Ent_GetClass(ent) ~= "starfall_prop" then
			SF.Throw("The entity isn't a custom prop", 2)
		end
	...
	function ents_methods:setCustomPropShadowForce(data)
		...
		if data then
			...
			if data.teleportdistance > 0 and not haspermission(instance, ent, "entities.setPos") then
				SF.Throw(
					"Shadow force property 'teleportdistance' higher than 0 requires 'entities.setPos' permission access",
					2
				)
			end

			checkluatype(data.secondstoarrive, TYPE_NUMBER)
			if data.secondstoarrive < 1e-3 then
				SF.Throw("Shadow force property 'secondstoarrive' cannot be lower than 0.001", 2)
			end
			checkluatype(data.dampfactor, TYPE_NUMBER)
			if data.dampfactor > 1 or data.dampfactor < 0 then
				SF.Throw("Shadow force property 'dampfactor' cannot be higher than 1 or lower than 0", 2)
			end
	...
	function ents_methods:remove()
		local ent = getent(self)
		if Ent_IsWorld(ent) or Ent_IsPlayer(ent) then
			SF.Throw("Cannot remove world or player", 2)
		end
	...
	function ents_methods:breakEnt()
		local ent = getent(self)
		local ent_tbl = Ent_GetTable(ent)
		if Ent_IsPlayer(ent) or ent_tbl.WasBroken then
	...
	function ents_methods:use(usetype, value)
		...
		if usetype ~= nil then
			checkluatype(usetype, TYPE_NUMBER)
		end
		if value ~= nil then
			checkluatype(value, TYPE_NUMBER)
		end
		if Ply_InVehicle(instance.player) and Ent_IsVehicle(ent) then
			return
		end -- Prevent source engine bug when using vehicle while in a vehicle
	...
	function ents_methods:setCollisionGroup(group)
		...
		if group < 0 or group >= LAST_SHARED_COLLISION_GROUP then
			SF.Throw("Invalid collision group value", 2)
		end
		local ent = getent(self)
		if Ent_IsPlayer(ent) then
			SF.Throw("Target is a player!", 2)
	...
	function ents_methods:enableSphere(enabled, radius)
		...
		if Ent_GetClass(ent) ~= "prop_physics" then
			SF.Throw("This function only works for prop_physics", 2)
		end
		local phys = Ent_GetPhysicsObject(ent)
		if not Phys_IsValid(phys) then
			SF.Throw("Physics object is invalid", 2)
	...
	function ents_methods:setTrails(startSize, endSize, length, material, color, attachmentID, additive)
		...
		local time = CurTime()
		if ent_tbl._lastTrailSet == time then
			SF.Throw("Can't modify trail more than once per frame", 2)
		end
		ent_tbl._lastTrailSet = time

		if string.find(material, '"', 1, true) then
			SF.Throw("Invalid Material", 2)
		end
		checkpermission(instance, ent, "entities.setRenderProperty")
	...
	function ents_methods:setUnbreakable(on)
		...
		checkpermission(instance, ent, "entities.canTool")
		if Ent_GetClass(ent) ~= "prop_physics" then
			SF.Throw("setUnbreakable can only be used on prop_physics", 2)
		end
	...
	function ents_methods:setTriggerListener(startTouchCB, endTouchCB)
		...
		local class = Ent_GetClass(ent)
		if not physUpdateWhitelist[class] then
			SF.Throw("Cannot use trigger listener on " .. class, 2)
		end
	...
	function ents_methods:setLightingOriginEntity(lightOrigin)
		...
		checkpermission(instance, ent, "entities.setRenderProperty")
		if lightOrigin then
			lightOrigin = getent(lightOrigin)
		end

...
local Ent_GetNWEntity, Ent_GetTable, Ent_IsValid, Ent_SetNWEntity =
	ENT_META.GetNWEntity, ENT_META.GetTable, ENT_META.IsValid, ENT_META.SetNWEntity
...
if CPPI then
	...
	if SERVER then
		P.checks = {
			function(instance, target)
				if Ent_IsValid(target) then
					if target == instance.player and owneraccess:GetBool() then
						return true
					end
					if Ply_IsSuperAdmin(instance.player) then
						return true
					end
			...
			function(instance, target)
				if Ent_IsValid(target) then
					if
						...
						or LocalPlayer() == instance.player
						or Ply_IsSuperAdmin(instance.player)
					then
						return true
					end
					if target:CPPIGetOwner() == instance.player then
		...
		if not ENT_META.CPPICanTool then
			P.checks[2] = P.checks[1]
		end
		if not ENT_META.CPPICanPhysgun then

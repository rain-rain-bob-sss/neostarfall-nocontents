	...
	if self.index > #self then -- Most often case
		...
		while length > 0 do
			if self.index > #self then -- End of buffer
				...
				self[self.index] = string.sub(cur, 1, self.subindex - 1)
					.. string.sub(data, i, i + sublength - 1)
					.. string.sub(cur, self.subindex + sublength)
...
function ss_methods:seek(pos)
	if pos < 1 then
		error("Index must be 1 or greater", 2)
	end
	self.index = #self + 1
	self.subindex = 1
...
function ss_methods:writeInt8(x)
	if x == math_huge or x == -math_huge or x ~= x then
		error("Can't convert error float to integer!", 2)
	end
	if x < 0 then
		x = x + 0x100
	end
...
return function(instance)
	...
	function bit_library.decompress(s, maxSize)
		...
		if #s <= 13 then
			return nil
		end -- Size of header is 13 bytes, so it can't possibly have any data if it's that size or smaller.

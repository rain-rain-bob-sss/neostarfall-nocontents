...
net.Receive("SF_netmessage", function(len, ply)
	...
	if IsValid(ent) then
		...
		if instance and instance.runScriptHook then
			...
			if ply then
				ply = instance.Types.Player.Wrap(ply)
			end
...
return function(instance)
	...
	function net_library.start(name)
		...
		if netStarted then
			SF.Throw("net message was already started", 2)
		end
	...
	function net_library.send(target, unreliable)
		if unreliable ~= nil then
			checkluatype(unreliable, TYPE_BOOL)
		end
		if not netStarted then
	...
	function net_library.readStream(cb)
		...
		if streams[instance.player] then
			SF.Throw("The previous stream must finish before reading another.", 2)
		end
	...
	function net_library.cancelStream()
		if not streams[instance.player] then
			SF.Throw("Not currently reading a stream.", 2)
		end

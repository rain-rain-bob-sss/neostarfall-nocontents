timer.Simple(0, function()
	if util.NetworkStringToID("sf_moneyrequest") ~= 0 then
		if SERVER then
			ErrorNoHalt(
				"NSF: loganlearner/starfall-darkrp-library is obsolete as Neostarfall now has a built-in DarkRP library. Please uninstall loganlearner/starfall-darkrp-library\n"
			)
		else
			print(
				"NSF: This server has loganlearner/starfall-darkrp-library installed, which is obsolete as Neostarfall now has a built-in DarkRP library. The built-in library will be disabled. If you experience any problems with DarkRP-specific code, this might be why!"
			)
		end
	end
end)
...
if SERVER then
	...
	givemoneyBurst = SF.BurstObject(
		"givemoney",
		"money giving",
		0.5,
		2,
		"The rate at which the cooldown for giving out that can be made for a single player decreases per second. Lower is longer, higher is shorter.",
		"Number of times a single player can give out money in a short time."
	)
	moneyrequestBurst = SF.BurstObject(
		"moneyrequest",
		"money request",
		0.5,
		1,
		"The rate at which the cooldown for money requests that can be made for a single player decreases per second. Lower is longer, higher is shorter.",
		"Number of money requests that can be made by a single player in a short time."
	)

	registerprivilege(
		"darkrp.moneyPrinterHooks",
		"Get own money printer info",
		"Allows the user to know when their own money printers catch fire or print money (and how much was printed)"
	)
	registerprivilege(
		"darkrp.playerWalletChanged",
		"Be notified of wallet changes",
		"Allows the user to know when their own wallet changes"
	)
	registerprivilege(
		"darkrp.lockdownHooks",
		"Know when lockdowns begin and end",
		"Allows the user to know when a lockdown begins or ends"
	)
	registerprivilege(
		"darkrp.lawHooks",
		"Know when laws change",
		"Allows the user to know when a law is added or removed, and when the laws are reset"
	)
	registerprivilege(
		"darkrp.lockpickHooks",
		"Know when they start picking a lock",
		"Allows the user to know when they start or finish lockpicking"
	)
	registerprivilege(
		"darkrp.requestMoney",
		"Ask players for money",
		"Allows the user to prompt other users for money (similar to E2 moneyRequest)"
	)
	...
	function manager:add(sender, receiver, amount, message, instance, callbackSuccess, callbackFailure)
		if next(self.requests) == nil then
			hook.Add("Think", "SF_DarkRpMoneyRequests", function()
				self:think()
			end)
		end
		...
		local request =
			requestClass:new(sender, receiver, amount, message, expiry, instance, callbackSuccess, callbackFailure)
	...
	function manager:pop(sender, receiver, force)
		...
		if not force and (request and request.expiry < CurTime()) then
			return
		end
		...
		if next(requestsForPlayer) == nil then
			...
			if next(self.requests) == nil then
				hook.Remove("Think", "SF_DarkRpMoneyRequests")
			end
		end
	...
	concommand.Add(
		...
		function(executor, command, args)
			...
			if IsValid(executor) and sender ~= executor and not executor:IsSuperAdmin() then
				return chatPrint(
					executor,
					"sf_moneyrequest: only superadmins can interact with other people's money requests"
				)
			end
		...
		nil,
		"Accept, decline, or view info about a Neostarfall money request. Usage: sf_moneyrequest <entindex or 0> <accept|decline|info> <request index>",
	...
	local function createMoneyRequestPanel(receiver, amount, expiry, message)
		...
		desc:SetText(
			string.format(
				"A Neostarfall processor owned by %q (%s) is asking you for %s. Would you like to send them money?",
				receiver:GetName(),
				receiver:SteamID(),
...
if SERVER then
	...
	SF.hookAdd("moneyPrinterCatchFire", nil, function(instance, moneyprinter)
		if not moneyprinter then
			return false
		end
		if instance.player ~= SF.Superuser then
			if not moneyprinter.Getowning_ent or instance.player ~= moneyprinter:Getowning_ent() then
				return false
			end
			if not SF.Permissions.hasAccess(instance, nil, "darkrp.moneyPrinterHooks") then
				return false
			end
		end
	...
	SF.hookAdd("moneyPrinterPrinted", nil, function(instance, moneyprinter, moneybag)
		if not moneyprinter or not moneybag then
			return false
		end
		if instance.player ~= SF.Superuser then
			if not moneyprinter.Getowning_ent or instance.player ~= moneyprinter:Getowning_ent() then
	...
	SF.hookAdd("moneyPrinterPrintMoney", nil, function(instance, moneyprinter, amount)
		if not checksafety(amount) then
			return false
		end
		if not moneyprinter then
			return false
	...
	SF.hookAdd("playerWalletChanged", nil, function(instance, ply, amount, wallet)
		if not checksafety(amount, wallet) then
			return false
		end
		if instance.player ~= SF.Superuser then
			if not SF.Permissions.hasAccess(instance, nil, "darkrp.playerWalletChanged") then
				return false
			end
			if instance.player ~= ply then
				return false
			end
		end
	...
	SF.hookAdd("lockdownEnded", nil, function(instance, actor)
		if instance.player ~= SF.Superuser and not SF.Permissions.hasAccess(instance, nil, "darkrp.lockdownHooks") then
			return false
		end
	...
	SF.hookAdd("addLaw", nil, function(instance, index, law, player)
		if not checksafety(index, law) then
			return false
		end
		if instance.player ~= SF.Superuser and not SF.Permissions.hasAccess(instance, nil, "darkrp.lawHooks") then
			return false
		end
	...
	SF.hookAdd("lockpickStarted", nil, function(instance, ply, ent, trace)
		if instance.player ~= SF.Superuser then
			if not SF.Permissions.hasAccess(instance, nil, "darkrp.lockpickHooks") then
				return false
			end
			if instance.player ~= ply then
				return false
	...
	SF.hookAdd("onLockpickCompleted", nil, function(instance, ply, success, ent)
		if not checksafety(success) then
			return false
		end
		if instance.player ~= SF.Superuser then
			if not SF.Permissions.hasAccess(instance, nil, "darkrp.lockpickHooks") then
...
return function(instance)
	...
	if SERVER then
		...
		function darkrp_library.payPlayer(sender, receiver, amount)
			...
			if amount <= 0 or amount >= (2 ^ 32) then
				SF.Throw("amount must be positive", 2)
				return
			end
			checkpermission(instance, nil, "darkrp.giveMoney")
			sender = getply(sender)
			if instance.player ~= SF.Superuser and instance.player ~= sender then
				SF.Throw("may not transfer money from player other than owner", 2)
				return
			end
			if sender:canAfford(amount) then
		...
		function darkrp_library.requestMoney(sender, amount, message, callbackSuccess, callbackFailure, receiver)
			...
			if callbackFailure ~= nil then
				checkluatype(callbackFailure, TYPE_FUNCTION)
			end
			if message ~= nil then
				checkluatype(message, TYPE_STRING)
				if #message > 60 then
					SF.Throw("Money request message may not exceed 60 bytes", 2)
				end
			end
			amount = math.ceil(amount)
			if amount <= 0 or amount >= (2 ^ 32) then
			...
			if instance.player == SF.Superuser then
				...
				if receiver ~= nil then
					SF.Throw("Cannot use receive argument if not superuser", 2)
				end
				receiver = instance.player
			end

			if manager:exists(sender, receiver) then
				SF.Throw("You already have a pending request for this sender", 2)
			end
		...
		function darkrp_library.canMakeMoneyRequest(sender)
			if moneyrequestBurst:check(instance.player) < 1 then
				return false
			end
			if sender ~= nil then
				if manager:exists(getply(sender), instance.player) then
					return false
				end
			end
			return true
		...
		function darkrp_library.openF1Menu()
			if instance.player ~= SF.Superuser and instance.player ~= LocalPlayer() then
				SF.Throw("may not use this function on anyone other than owner", 2)
				return
			end
			DarkRP.openF1Menu()
		end
	...
	function darkrp_library.getShipmentContents(ent)
		ent = getent(ent)
		if not CustomShipments or not ent.Getcontents then
			return
		end
	...
	if SERVER then
		...
		function darkrp_library.keysUnOwnAll(ply)
			ply = getply(ply)
			if instance.player ~= SF.Superuser and instance.player ~= ply then
				SF.Throw("may not use this function on anyone other than owner", 2)
				return
			end
	...
	function darkrp_library.getDarkRPVar(ply, k)
		checkluatype(k, TYPE_STRING)
		if instance.player ~= SF.Superuser and SF.BlacklistedDarkRPVars[k] then
			return
		end
	...
	function darkrp_library.getMoney(ply)
		if instance.player ~= SF.Superuser and SF.BlacklistedDarkRPVars.money then
			return
		end

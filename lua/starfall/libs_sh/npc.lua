...
if SERVER then
	...
	lagCompensated = SF.EntManager(
		"npcs_lag_compensated",
		"lag compensated npcs",
		40,
		"The number of npcs allowed to be lag compensated"
	)
end
...
return function(instance)
	...
	local Ent_GetClass, Ent_IsLagCompensated, Ent_IsValid, Ent_Remove, Ent_SetLagCompensated =
		ENT_META.GetClass, ENT_META.IsLagCompensated, ENT_META.IsValid, ENT_META.Remove, ENT_META.SetLagCompensated
	local Npc_AddEntityRelationship, Npc_AddRelationship, Npc_GetEnemy, Npc_Give, Npc_SetLastPosition, Npc_SetSchedule, Npc_SetTarget =
		NPC_META.AddEntityRelationship,
		NPC_META.AddRelationship,
		NPC_META.GetEnemy,
		NPC_META.Give,
		NPC_META.SetLastPosition,
		NPC_META.SetSchedule,
		NPC_META.SetTarget

	local owrap, ounwrap = instance.WrapObject, instance.UnwrapObject
	local npc_methods, npc_meta, wrap, unwrap =
		instance.Types.Npc.Methods, instance.Types.Npc, instance.Types.Npc.Wrap, instance.Types.Npc.Unwrap
	...
	if SERVER then
		...
		function npc_methods:addEntityRelationship(ent, disp, priority)
			...
			if not relation then
				SF.Throw("Invalid relationship specified", 2)
			end
			checkpermission(instance, npc, "npcs.modify")
		...
		function npc_methods:giveWeapon(wep)
			...
			if Ent_IsValid(weapon) then
				if Ent_GetClass(weapon) == "weapon_" .. wep then
					return
				end
				Ent_Remove(weapon)
			end

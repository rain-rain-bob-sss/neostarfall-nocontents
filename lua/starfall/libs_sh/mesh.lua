...
function SF.ParseObj(obj, thread_yield, Vector, triangulate)
	...
	local map = {
		v = function(f)
			pos[#pos + 1] = Vector(tonumber(f()), tonumber(f()), tonumber(f()))
		end,
		vt = function(f)
			uv[#uv + 1] = tonumber(f())
			uv[#uv + 1] = 1 - tonumber(f())
		end,
		vn = function(f)
			norm[#norm + 1] = Vector(tonumber(f()), tonumber(f()), tonumber(f()))
		end,
	}
	...
	while true do
		...
		for line in lines do
			...
			if tag and not ignore[tag] then
				...
				if t then
					...
					if not ok then
						SF.Throw("Failed to parse tag: (" .. line .. ") " .. err, 3)
					end
				else
			...
			if thread_yield and linen % 100 == 0 then
				thread_yield()
			end
			linen = linen + 1
		end
		...
		if not name then
			SF.Throw("The .obj group/object name is missing!", 3)
		end
		if #face < 3 or #face % 3 ~= 0 then
			SF.Throw("Expected a multiple of 3 vertices for the mesh's triangles.", 3)
		end
		...
		for k, v in ipairs(face) do
			...
			vertices[k] = vert
			if thread_yield and k % 100 == 0 then
				thread_yield()
			end
		end
...
function SF.GenerateTangents(vertices, thread_yield, Vector)
	...
	for i = 1, #vertices do
		tan1[i] = Vector(0, 0, 0)
		tan2[i] = Vector(0, 0, 0)
		if thread_yield and i % 100 == 0 then
			thread_yield()
		end
	end
...
do
	...
	function update_points(points)
		...
		for k, point in ipairs(points) do
			if not isValid(point.ent) then
				goto cont
			end
			local cur_pos = point.ent:getPos()
			if not changed and (not point.vec or point.vec ~= cur_pos) then
				changed = true
			end
	...
	function create_initial_simplex4(points, thread_yield)
		...
		for i = 2, #points do
			...
			if i % 100 == 0 and thread_yield then
				thread_yield()
			end
		end
		...
		for i = 1, #points do
			local p = points[i]
			if p == point1 or p == point2 or p == point3 then
				goto cont
			end
	...
	function quickhull(points, thread_yield)
		...
		while #face_stack > 0 do
			...
			repeat
				...
				if #horizon_edges % 100 == 0 and thread_yield then
					thread_yield()
				end
			...
			iter = iter + 1
			if iter % 10 == 0 and thread_yield then
				thread_yield()
			end
		...
		while l.value do
			local face = l.value
			l = l.next
			if face.lightface then
				goto cont
			end -- Filter out invalid faces

			for k, vert in ipairs({ face_vertices(face) }) do
				local point = vert.point
				if ret_points_added[point] then
					goto cont
				end
			...
			if #ret_faces % 100 == 0 and thread_yield then
				thread_yield()
			end
			::cont::
		end
...
-- Register privileges
SF.Permissions.registerPrivilege(
	"mesh",
	"Create custom mesh",
	"Allows users to create custom meshes for rendering.",
	{ client = {} }
)

-- 1M triangles is about 195.4M VRAM
local plyTriangleCount = SF.LimitObject(
	"mesh_triangles",
	"total mesh triangles",
	1000000,
	"How many triangles total can be loaded for meshes."
)
local plyTriangleRenderBurst = SF.BurstObject(
	"mesh_triangles",
	"rendered triangles",
	50000,
	50000,
	"Number of triangles that can be rendered per frame",
	"Number of triangles that can be drawn in a short period of time"
)
...
return function(instance)
	...
	function mesh_library.parseObj(obj, threaded, triangulate)
		...
		if triangulate ~= nil then
			checkluatype(triangulate, TYPE_BOOL)
		end
	...
	function mesh_library.generateNormals(vertices, inverted, smooth_limit)
		...
		if inverted ~= nil then
			checkluatype(inverted, TYPE_BOOL)
		else
			inverted = false
		end
		if smooth_limit ~= nil then
			checkluatype(smooth_limit, TYPE_NUMBER)
		else
			smooth_limit = 0
		end
		local nvertices = #vertices
		if nvertices < 3 or nvertices % 3 ~= 0 then
			SF.Throw("Expected a multiple of 3 vertices.", 2)
		end
	...
	function mesh_library.findConvexHull(vertices, threaded)
		...
		local nvertices = #vertices
		if nvertices < 4 then
			SF.Throw("Expected at least 4 vertices.", 2)
		end
	...
	if CLIENT then
		...
		local mesh_methods, mesh_meta, wrap, unwrap =
			instance.Types.Mesh.Methods, instance.Types.Mesh, instance.Types.Mesh.Wrap, instance.Types.Mesh.Unwrap

		local vertexCheck = {
			color = function(v)
				return dgetmeta(v) == col_meta
			end,
			normal = function(v)
				return dgetmeta(v) == vec_meta
			end,
			tangent = function(v)
				return dgetmeta(v) == vec_meta
			end,
			...
			v = isnumber,
			userdata = function(v)
				return istable(v) and isnumber(v[1]) and isnumber(v[2]) and isnumber(v[3]) and isnumber(v[4])
			end,
		}
		...
		function mesh_library.createFromTable(vertices, threaded)
			...
			local nvertices = #vertices
			if nvertices < 3 or nvertices % 3 ~= 0 then
				SF.Throw("Expected a multiple of 3 vertices for the mesh's triangles.", 2)
			end
			if nvertices > 65535 then
				SF.Throw("The max number of vertices for a mesh is 65535.", 2)
			end
			...
			for i, vertex in ipairs(vertices) do
				...
				unwrapped[i] = vert
				if threaded and i % 100 == 0 and i < #vertices then
					thread_yield(#vertices - i)
				end
			end
		...
		local function wrapVertex(p)
			local tri = {}
			if p.color then
				tri.color = cwrap(p.color)
			end
			tri.normal = vwrap(p.normal)
			tri.tangent = vwrap(p.tangent)
			if p.binormal then
				tri.binormal = vwrap(p.binormal)
			end
			tri.pos = vwrap(p.pos)
			tri.u = p.u
			tri.v = p.v
		...
		function mesh_library.getModelMeshes(model, lod, bodygroupMask)
			...
			if lod ~= nil then
				checkluatype(lod, TYPE_NUMBER)
			end
			if bodygroupMask ~= nil then
				checkluatype(bodygroupMask, TYPE_NUMBER)
			end
		...
		local prim_triangles = {
			[MATERIAL_LINES] = function(count)
				return (count * 2) / 3
			end,
			[MATERIAL_LINE_LOOP] = function(count)
				return count / 3
			end,
			[MATERIAL_LINE_STRIP] = function(count)
				return (count + 1) / 3
			end,
			-- Disabled since it seems to crash the game
			-- [MATERIAL_POINTS] = function(count) return count / 3 end,
			[MATERIAL_POLYGON] = function(count)
				return count - 2
			end,
			[MATERIAL_QUADS] = function(count)
				return count * 2
			end,
			[MATERIAL_TRIANGLES] = function(count)
				return count
			end,
			[MATERIAL_TRIANGLE_STRIP] = function(count)
				return count
			end,
		}
		...
		function mesh_library.generate(mesh_obj, prim_type, prim_count, func)
			if meshgenerating then
				SF.Throw("Dynamic mesh was already started.", 2)
			end
			...
			if not prim_trifunc then
				SF.Throw("Invalid Primitive.", 2)
			end

			if prim_count < 1 or prim_count ~= prim_count then
				SF.Throw("Can't generate with less than 1 primitive", 2)
			end
			-- Additional check to prevent crash with MATERIAL_POLYGON primitive count set to 1
			if prim_type == MATERIAL_POLYGON and prim_count < 2 then
				SF.Throw("Can't generate polygon with less than 2 primitive", 2)
			end
			if prim_count > 8192 then
				SF.Throw("Can't generate more than 8192 primitives", 2)
			end
			...
			if mesh_obj == nil then
				...
				if not mesh_tbl then
					SF.Throw("Tried to use invalid mesh.", 2)
				end
				-- Garrysmod bug, crash if mesh isn't empty
				if mesh_tbl.ntriangles ~= 0 then
					SF.Throw("mesh.generate requires an empty mesh to populate.", 2)
				end
		...
		function mesh_methods:destroy()
			local mesh = unwrap(self)
			if not meshData[mesh] then
				SF.Throw("Tried to use invalid mesh.", 2)
			end
			if meshgenerating == mesh then
				SF.Throw("Cannot destroy mesh currently being generated.", 2)
			end
			destroyMesh(instance.player, mesh)
		end
	end

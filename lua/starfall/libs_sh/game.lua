...
if SERVER then
	registerprivilege(
		"game.blastDamage",
		"Create explosions",
		"Allows the user to create explosions",
...
local fireBulletsBurst = SERVER
	and SF.BurstObject(
		...
		"Number of bullets that can be fired in a short time"
	)
local fireBulletsDPSBurst = SERVER
	and SF.BurstObject(
		"bullets_damage",
		"bullet damage",
		100,
		100,
		" bullets damage per second",
		"Damage per second bullets can deal"
	)
...
return function(instance)
	...
	local ent_methods, ent_meta, ewrap, eunwrap =
		instance.Types.Entity.Methods, instance.Types.Entity, instance.Types.Entity.Wrap, instance.Types.Entity.Unwrap
	local vec_methods, vec_meta, vwrap, vunwrap =
		instance.Types.Vector.Methods, instance.Types.Vector, instance.Types.Vector.Wrap, instance.Types.Vector.Unwrap
	...
	function game_library.modelExists(path)
		...
		if string.find(path, "models/", 1, true) ~= 1 then
			return false
		end
	...
	if SERVER then
		...
		function game_library.blastDamage(damageOrigin, damageRadius, damage)
			...
			util.BlastDamage(
				instance.entity,
				instance.player,
				vunwrap1(damageOrigin),
				math.Clamp(damageRadius, 0, 1500),
				damage
			)
		end
		...
		function game_library.bulletDamage(src, dir, damage, num, force, distance, spread, hullSize, ignoreEntity, cb)
			...
			dir = vunwrap2(dir)
			if spread ~= nil then
				spread = vunwrap3(spread)
			end
			if damage ~= nil then
				checkluatype(damage, TYPE_NUMBER)
				damage = math.Clamp(damage, 1, fireBulletsDPSBurst.max)
			end
			if force ~= nil then
			...
			if distance ~= nil then
				...
				distance = math.Clamp(distance, 0, 56756)
			end
			if hullSize ~= nil then
				checkluatype(hullSize, TYPE_NUMBER)
				hullSize = math.Clamp(hullSize, 0, maxBulletHull:GetInt())
			end
			if num ~= nil then
				checkluatype(num, TYPE_NUMBER)
				num = math.Clamp(num, 1, fireBulletsBurst.max)
			end
			if ignoreEntity ~= nil then
				ignoreEntity = eunwrap(ignoreEntity)
			end
			...
			if cb then
				...
				callback = function(attacker, tr, dmginfo)
					instance:runFunction(
						cb,
						instance.WrapObject(attacker),
						SF.StructWrapper(instance, tr, "TraceResult")
					)
				end
			end
		...
		function game_library.canFireBullets(damage, num)
			...
			checkluatype(num, TYPE_NUMBER)
			return (
				fireBulletsBurst:check(instance.player) >= num
				and fireBulletsDPSBurst:check(instance.player) >= damage * num
			)
		end

...
registerprivilege(
	"hologram.create",
	"Create hologram",
	"Allows the user to create holograms",
	CLIENT and { client = {} } or nil
)
registerprivilege(
	"hologram.setRenderProperty",
	"RenderProperty",
	"Allows the user to change the rendering of an entity",
	{ entities = {} }
)

local entList = SF.EntManager(
	"holograms",
	"holograms",
	200,
...
SF.ResourceCounters.Holograms = {
	icon = "icon16/bricks.png",
	count = function(ply)
		return entList:get(ply)
	end,
}

local cl_hologram_meta_overrides = {
	CPPIGetOwner = function(ent)
		return Ent_GetTable(ent).SFHoloOwner
	end,
	CPPICanTool = function(ent, pl)
		return Ent_GetTable(ent).SFHoloOwner == pl
	end,
	CPPICanPhysgun = function(ent, pl)
		return Ent_GetTable(ent).SFHoloOwner == pl
	end,
}
local cl_hologram_meta = {
	__index = function(ent, k)
		return cl_hologram_meta_overrides[k] or ENT_META.__index(ent, k)
	end,
...
if SERVER then
	registerprivilege(
		"hologram.setMoveType",
		"Set MoveType",
		"Allows the user to set hologram's movetype",
		{ entities = {} }
	)
end
...
return function(instance)
	...
	local Ent_AddEffects, Ent_DisableMatrix, Ent_DrawModel, Ent_EnableMatrix, Ent_GetColor4Part, Ent_GetTable, Ent_IsValid, Ent_LookupSequence, Ent_OBBMaxs, Ent_OBBMins, Ent_RemoveEffects, Ent_ResetSequence, Ent_SetAngles, Ent_SetCycle, Ent_SetLocalAngles, Ent_SetLocalAngularVelocity, Ent_SetLocalPos, Ent_SetLocalVelocity, Ent_SetModel, Ent_SetMoveType, Ent_SetPlaybackRate, Ent_SetPos, Ent_SetupBones, Ent_Spawn =
		ENT_META.AddEffects,
		ENT_META.DisableMatrix,
		ENT_META.DrawModel,
		ENT_META.EnableMatrix,
		...
		ENT_META.OBBMaxs,
		ENT_META.OBBMins,
		ENT_META.RemoveEffects,
		ENT_META.ResetSequence,
		ENT_META.SetAngles,
		ENT_META.SetCycle,
		...
		ENT_META.SetPlaybackRate,
		ENT_META.SetPos,
		ENT_META.SetupBones,
		ENT_META.Spawn

	local hologram_library = instance.Libraries.hologram
	local hologram_methods, hologram_meta, wrap, unwrap =
		instance.Types.Hologram.Methods,
		instance.Types.Hologram,
		instance.Types.Hologram.Wrap,
		instance.Types.Hologram.Unwrap
	...
	function ents_methods:toHologram()
		...
		if not Ent_GetTable(ent).IsSFHologram then
			SF.Throw("The entity isn't a hologram", 2)
		end
		return wrap(ent)
	end
	...
	function hologram_library.create(pos, ang, model, scale)
		...
		if SERVER then
			...
			if instance.data.render.isRendering then
				SF.Throw("Can't create hologram while rendering!", 2)
			end
		...
		if Ent_IsValid(holoent) then
			...
			if SERVER then
				if CPPI then
					holoent:CPPISetOwner(ply == SF.Superuser and NULL or ply)
				end
			else
	...
	function hologram_library.canSpawn()
		if not SF.Permissions.hasAccess(instance, nil, "hologram.create") then
			return false
		end
	...
	if SERVER then
		...
		function hologram_methods:setLocalAngularVelocity(angvel)
			...
			local sfParent = SF.Parent.Get(holo)
			if sfParent then
				sfParent.localAngVel:Set(angvel)
			end
		end
	...
	function hologram_methods:setModel(model)
		...
		if
			(SERVER and not util.IsValidModel(model)) or (CLIENT and string.GetExtensionFromFilename(model) ~= "mdl")
		then
	...
	function hologram_methods:setRenderGroup(group)
		...
		if group then
			...
			if not SF.allowedRenderGroups[group] then
				SF.Throw("Invalid rendergroup!")
			end
	...
	function hologram_methods:remove()
		if CLIENT and instance.data.render.isRendering then
			SF.Throw("Cannot remove while in rendering hook!", 2)
		end
		local holo = getholo(self)

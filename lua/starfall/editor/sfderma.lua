...
function PANEL:Init()
	...
	self.TitleBar.OnMousePressed = function(_, ...)
		self:OnMousePressed(...)
	end
...
local function moveFile(fileNode, toNode)
	if toNode:GetFileName() then
		return false
	end
	if fileNode == toNode then
		return false
	end
	local sourcePath
	...
	if file.Exists(toNode:GetFolder() .. "/" .. sourceName, "Data") then
		SF.AddNotify(
			LocalPlayer(),
			"Failed to move " .. sourceName .. ", it already exists in: " .. toNode:GetFolder(),
			"ERROR",
			7,
			"ERROR1"
		)
...
function PANEL:Setup(folder)
	...
	for k, v in pairs({
		...
		{
			"HttpQueue",
			"https://raw.githubusercontent.com/ANormalTwig/PublicStarfalls/main/libraries/http_queueing.lua",
		},
		{
			"ReadWriteType",
			"https://raw.githubusercontent.com/Jacbo1/Public-Starfall/main/ReadWriteType/readwritetype.lua",
		},
	...
	http.Fetch(examples_url, function(body, len, headers, code)
...
local function sort(tbl)
	...
	for k, v in pairs(tbl) do
		sorted[#sorted + 1] = { string.lower(v), v }
	end
...
function PANEL:AddFiles(filter, keepExpanded)
	if self.Root.ChildNodes then
		self.Root.ChildNodes:Clear()
	end
...
function PANEL:DoRightClick(node)
	...
	if menu == "file" then
		...
		self.menu:AddOption("Rename", function()
			Derma_StringRequestNoBlur("Rename file", "", string.StripExtension(node:GetText()), function(text)
				if text == "" then
					return
				end
				...
				if not string.match(saveFile, "%.txt$") then
		...
		self.menu:AddOption("Delete", function()
			Derma_Query("Are you sure you want to delete this file?", "Delete file", "Delete", function()
	...
	elseif menu == "folder" then
		...
		self.menu:AddOption("New file", function()
			Derma_StringRequestNoBlur("New file", "", "", function(text)
				if text == "" then
					return
				end
		...
		self.menu:AddOption("New folder", function()
			Derma_StringRequestNoBlur("New folder", "", "", function(text)
				if text == "" then
					return
				end
				text = string.GetFileFromFilename(string.gsub(text, ".", invalid_filename_chars))
		...
		self.menu:AddSpacer()
		self.menu:AddOption("Delete", function()
			Derma_Query("Are you sure you want to delete this folder?", "Delete folder", "Delete", function()
				-- Recursive delete
	...
	for area, set in ipairs(listedPerms) do
		...
		for id, _ in SortedPairs(set) do
			...
			perm.update = function()
				if area == 2 then
					perm.state = perm:GetToggle() and 2 or 3
				else
					perm.state = perm:GetToggle() and 2 or 1
				end
				if perm.initialState then
					local hint
					if perm.state ~= perm.initialState then
						hint = permStates[perm.state].changedText or ""
					else
						hint = permStates[perm.state].stillText or ""
					end
					perm:SetToolTip(id .. "\n" .. description .. "\n\n" .. hint)
					ChangeTooltip(perm)
				end
...
function PANEL:OpenForChip(chip, showOverrides)
	...
	self.description:SetText(
		#desc > 0 and desc
			or 'Please press "Grant" a couple of times. Then press "Apply Permissions", so this way we can provide you with interesting features.'
	)
	self.description:SetTooltip(
		"Description attached to permission request from\n"
			.. chip.owner:GetName()
			.. " [ "
			.. chip.owner:SteamID()
			.. " ]"
...
function PANEL:Init()
	...
	notice.update = function()
		notice:SetText(
			notice.prefixes[self.area]
				.. " They might be useful to touch advanced technologies. There is place for any influence rendered by their features."
		)
	end
	...
	warning:SetText(
		"State of permissions listed below will override global settings. Grant overrides only if you trust the owner."
	)
	...
	showOverrides.update = function()
		showOverrides:SetVisible(
			self.area == 1
				and self.chip.instance.permissionOverrides
				and table.Count(self.chip.instance.permissionOverrides) > 0
		)
	end
...
function PANEL:GetFont(tab)
	local name = string.format(
		"sf_fonteditor_%s_%d_%d_%d_%d%d%d%d%d",
		tab.font or "Arial",
		tonumber(tab.size),
		tonumber(tab.weight),
...
function PANEL:BuildFontString(tab, pretty, tips, prependLocalVariable)
    ...
    %s%s
			...
			prepend,
			tab.font,
			tips and " -- Font name" or "",
			tab.size,
			tips and " -- Size" or "",
			tab.weight,
			tips and " -- Weight (how bold)" or "",
			tab.antialias,
			tips and " -- Antialias" or "",
			tab.additive,
			tips and " -- Additive" or "",
			tab.shadow,
			tips and " -- Shadow" or "",
			tab.outline,
			tips and " -- Outline" or "",
			tab.blursize,
			tips and " -- Blur size" or "",
			tab.extended,
			tips and " -- Extended (Allow more UTF8 chars)" or "",
			tab.scanlines,
			tips and " -- Scanlines" or ""
		)
	else
		-- one-liner
		return string.format(
			'%srender.createFont("%s",%s,%s,%s,%s,%s,%s,%s,%s,%s)',
			prepend,
			tab.font,
			tab.size,
			tab.weight,
...
function PANEL:ParseFontString(str)
	...
	local name, size, weight, antialias, additive, shadow, outline, blursize, extended, scanlines = string.match(
		str,
		string.format(
			"render.createFont%%s*%%(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s%%)",
			STRING,
			NUMBER,
			NUMBER,
			BOOLEAN,
...
function PANEL:Init()
	...
	for k, controlinfo in pairs(controls) do
		...
		if inp.SetDefaultValue then
			inp:SetDefaultValue(self.fontSettings[settingName])
		end
		controlinfo[5] = inp

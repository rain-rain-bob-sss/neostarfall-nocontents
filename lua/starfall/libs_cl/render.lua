...
registerprivilege(
	"render.screen",
	"Render Screen",
	"Allows the user to render to a neostarfall screen",
	{ client = {} }
)
...
registerprivilege(
	"render.renderView",
	"Render View",
	"Allows the user to render the world again with custom perspective",
	{ client = {} }
)
registerprivilege(
	"render.renderscene",
	"Render Scene",
	"Allows the user to render a world again without a screen with custom perspective",
	{ client = {} }
)
registerprivilege(
	"render.effects",
	"Render Effects",
	"Allows the user to render special effects such as screen blur, color modification, and bloom",
	{ client = {} }
)
...
registerprivilege(
	...
	"Allows capturing a rendertarget into an image format",
	{ client = { default = 1 } }
)
...
local rt_bank = SF.ResourceHandler(
	"render_rendertargets",
	"Render targets",
	"20",
	"The max number of user created rendertargets",
	function(_, i)
...
local pixhandle_bank = SF.ResourceHandler(
	"render_pixvishandlesperframe",
	"Pixvis handles",
	"50",
	"How many render.isPixelVisible can be called per frame",
	function()
...
local function cleanupRenderAllowTrueReturn(instance, args)
	instance:cleanupRender()
	if args[1] and args[2] == true then
		return true
	end
end
...
local function canRenderHud(instance)
	return SF.IsHUDActive(instance.entity)
		and (haspermission(instance, nil, "render.hud") or instance.player == SF.Superuser)
end

local function hudPrepareSafeArgs(instance, ...)
	if
		SF.IsHUDActive(instance.entity)
		and (haspermission(instance, nil, "render.hud") or instance.player == SF.Superuser)
	then
...
SF.hookAdd("HUDShouldDraw", nil, function(instance, ...)
	...
	if args[1] and args[2] == false then
		return false
	end
end)
...
SF.hookAdd("CalcView", nil, function(instance, ply, pos, ang, fov, znear, zfar)
	return SF.IsHUDActive(instance.entity)
		and (haspermission(instance, nil, "render.calcview") or instance.player == SF.Superuser),
	...
	if tbl[1] and istable(t) then
		...
		if t.origin then
			pcall(function()
				ret.origin = instance.Types.Vector.Unwrap(t.origin)
			end)
		end
		if t.angles then
			pcall(function()
				ret.angles = instance.Types.Angle.Unwrap(t.angles)
			end)
		end
		ret.fov = t.fov
		ret.znear = t.znear
...
return function(instance)
	...
	local matrix_meta, mwrap, munwrap =
		instance.Types.VMatrix, instance.Types.VMatrix.Wrap, instance.Types.VMatrix.Unwrap
	local markup_methods, markwrap, markunwrap =
		instance.Types.Markup.Methods, instance.Types.Markup.Wrap, instance.Types.Markup.Unwrap
	...
	instance:AddHook("initialize", function()
		...
		vunwrap1, vunwrap2, vunwrap3, vunwrap4 =
			vec_meta.QuickUnwrap1, vec_meta.QuickUnwrap2, vec_meta.QuickUnwrap3, vec_meta.QuickUnwrap4
	...
	function render_library.setStencilEnable(enable)
		...
		if renderdata.noStencil and not renderdata.usingRT then
			SF.Throw("Stencil operations must be used inside RenderTarget or HUD")
		end
	...
	function render_library.pushMatrix(transform, absolute)
		...
		if id + 1 > MATRIX_STACK_LIMIT then
			SF.Throw("Pushed too many matrices", 2)
		end
	...
	function render_library.popMatrix()
		...
		if #matrix_stack <= 0 then
			SF.Throw("Popped too many matrices", 2)
		end
	...
	function render_library.pushViewMatrix(tbl)
		...
		if #view_matrix_stack == MATRIX_STACK_LIMIT then
			SF.Throw("Pushed too many matrices", 2)
		end
		checkluatype(tbl, TYPE_TABLE)

		local newtbl = {}
		if tbl.origin ~= nil then
			newtbl.origin = vunwrap1(tbl.origin)
		end
		if tbl.angles ~= nil then
			newtbl.angles = aunwrap1(tbl.angles)
		end
	...
	local function gettexture(mat)
		if isstring(mat) then
			...
			if not rt then
				SF.Throw("Invalid Rendertarget", 3)
			end
			return rt
		else
	...
	function render_library.setMaterialEffectColorModify(mat, cmStructure)
		...
		for key, default in pairs(defaultCM) do
			...
			elseif TypeID(value) ~= TYPE_NUMBER then
				SF.Throw('Invalid type for key "' .. key .. '" (expected number, got ' .. SF.GetType(value) .. ")", 2)
			end
	...
	function render_library.drawBlurEffect(blurx, blury, passes)
		...
		if not renderdata.usingRT then
			SF.Throw("Cannot use this function outside of a rendertarget.", 2)
		end
	...
	function render_library.updateScreenEffectTexture(index)
		...
		if renderdata.updatedScreenTex then
			SF.Throw("Can't update screen effect textures more than once per render.", 2)
		end
	...
	function render_library.createRenderTarget(name)
		...
		if renderdata.rendertargets[name] then
			SF.Throw("A rendertarget with this name already exists!", 2)
		end
	...
	function render_library.selectRenderTarget(name)
		...
		if name then
			...
			local rt = renderdata.rendertargets[name]
			if not rt then
				SF.Throw("Invalid Rendertarget", 2)
			end
	...
	local drawTriangle
	do
		local mesh_Position, mesh_Color, mesh_AdvanceVertex = mesh.Position, mesh.Color, mesh.AdvanceVertex
		...
		drawTriangle = function(x1, y1, x2, y2, x3, y3)
			...
			local r, g, b, a = Col_Unpack(currentcolor)
			mesh_Position(v1_vec)
			mesh_Color(r, g, b, a)
			mesh_AdvanceVertex()
			mesh_Position(v2_vec)
			mesh_Color(r, g, b, a)
			mesh_AdvanceVertex()
			mesh_Position(v3_vec)
			mesh_Color(r, g, b, a)
			mesh_AdvanceVertex()
		end
	end
	...
	do
		...
		drawTexturedTriangleUV = function(vert1, vert2, vert3)
			...
			mesh_Position(v1_vec)
			mesh_Color(r, g, b, a)
			mesh_TexCoord(0, vert1.u or 0, vert1.v or 0)
			mesh_AdvanceVertex()
			mesh_Position(v2_vec)
			mesh_Color(r, g, b, a)
			mesh_TexCoord(0, vert2.u or 0, vert2.v or 0)
			mesh_AdvanceVertex()
			mesh_Position(v3_vec)
			mesh_Color(r, g, b, a)
			mesh_TexCoord(0, vert3.u or 0, vert3.v or 0)
			mesh_AdvanceVertex()
		end
	end
	...
	-- @param table dataG Green channel data.
	-- @param table dataB Blue channel data.
	function render_library.drawPixelsSubrectRGB(
		dstX,
		dstY,
		srcX,
		srcY,
	...
	-- @param table dataB Blue channel data.
	-- @param table dataA Alpha channel data.
	function render_library.drawPixelsSubrectRGBA(
		dstX,
		dstY,
		srcX,
		srcY,
	...
	-- \- Lucida Console
	-- \- Times New Roman
	function render_library.createFont(
		font,
		size,
		weight,
		antialias,
		...
		if not defined_fonts[name] then
			...
			if fontCount == cv_max_fonts:GetInt() then
				SF.Throw("You have reached the max created fonts!", 2)
			end
	...
	function render_library.overrideBlend(
		on,
		srcBlend,
		destBlend,
		blendFunc,
	...
	do
		...
		draw3DQuadUV = function(vert1, vert2, vert3, vert4)
			...
			mesh_Position(quad_v1)
			mesh_Color(r, g, b, a)
			mesh_TexCoord(0, vert1[4], vert1[5])
			mesh_AdvanceVertex()
			mesh_Position(quad_v2)
			mesh_Color(r, g, b, a)
			mesh_TexCoord(0, vert2[4], vert2[5])
			mesh_AdvanceVertex()
			mesh_Position(quad_v3)
			mesh_Color(r, g, b, a)
			mesh_TexCoord(0, vert3[4], vert3[5])
			mesh_AdvanceVertex()
			mesh_Position(quad_v4)
			mesh_Color(r, g, b, a)
			mesh_TexCoord(0, vert4[4], vert4[5])
			mesh_AdvanceVertex()
		end
	end
	...
	function render_library.cursorPos(ply, screen)
		if ply ~= nil then
			...
			if not ply:IsPlayer() then
				SF.Throw("Entity isn't a player", 2)
			end
		else
			ply = LocalPlayer()
		end
		...
		if not screen then
			SF.Throw("Invalid screen", 2)
		end
		local screenTransform = Ent_GetTable(screen).Transform
		if not screenTransform then
			SF.Throw("Invalid screen", 2)
		end
	...
	function render_library.capturePixels()
		...
		if not renderdata.usingRT then
			SF.Throw("Not in rendertarget context.", 2)
		end
	...
	function render_library.renderView(tbl)
		...
		if tbl.origin ~= nil then
			origin = vunwrap1(tbl.origin)
		end
		if tbl.angles ~= nil then
			angles = aunwrap1(tbl.angles)
		end
		if tbl.aspectratio ~= nil then
			checkluatype(tbl.aspectratio, TYPE_NUMBER)
		end
		if tbl.x ~= nil then
			checkluatype(tbl.x, TYPE_NUMBER)
		end
		if tbl.y ~= nil then
			checkluatype(tbl.y, TYPE_NUMBER)
		end
		if tbl.w ~= nil then
			checkluatype(tbl.w, TYPE_NUMBER)
			w = clamp(tbl.w, 1, 1024)
		end
		if tbl.h ~= nil then
			checkluatype(tbl.h, TYPE_NUMBER)
			h = clamp(tbl.h, 1, 1024)
		end
		if tbl.fov ~= nil then
			checkluatype(tbl.fov, TYPE_NUMBER)
		end
		if tbl.zfar ~= nil then
			checkluatype(tbl.zfar, TYPE_NUMBER)
		end
		if tbl.znear ~= nil then
			checkluatype(tbl.znear, TYPE_NUMBER)
		end
		if tbl.drawmonitors ~= nil then
			checkluatype(tbl.drawmonitors, TYPE_BOOL)
		end
		if tbl.drawviewmodel ~= nil then
			checkluatype(tbl.drawviewmodel, TYPE_BOOL)
		end
		...
		if tbl.dopostprocess ~= nil then
			checkluatype(tbl.dopostprocess, TYPE_BOOL)
		end
		if tbl.bloomtone ~= nil then
			checkluatype(tbl.bloomtone, TYPE_BOOL)
		end
		if tbl.znearviewmodel ~= nil then
			checkluatype(tbl.znearviewmodel, TYPE_NUMBER)
		end
		if tbl.zfarviewmodel ~= nil then
			checkluatype(tbl.zfarviewmodel, TYPE_NUMBER)
		end
		...
		if not renderdata.isScenic then
			SF.Throw("Can't use render.renderView outside of renderscene hook.", 2)
		end
	...
	function render_library.popCustomClipPlane()
		...
		if pushedClippingPlanes == 0 then
			SF.Throw("Popped too many clipping planes.", 2)
		end
	...
	function render_library.setScreenDimensions(screen, x, y, w, h)
		...
		if x - halfw < -1024 or y - halfh < -1024 or w < 1 or h < 1 or x + halfw > 1024 or y + halfh > 1024 then
			SF.Throw("The specified dimensions exceeds the bounds!", 2)
		end
		screen = getent(screen)
		local custominfo = SF.CustomScreenInfo
		if screen.ScreenInfo.Name ~= custominfo.Name then
			SF.Throw("Expected a custom screen. Make sure the selected screen is the 'custom screen' model!", 2)
		end
	...
	function render_library.screenShake(amplitude, frequency, duration)
		if not SF.IsHUDActive(instance.entity) then
			SF.Throw("Player isn't connected to HUD!", 2)
		end

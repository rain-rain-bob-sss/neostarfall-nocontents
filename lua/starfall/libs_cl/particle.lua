...
local plyEmitterCount = SF.LimitObject(
	"particleemitters",
	"particle emitters",
	8,
...
SF.ResourceCounters.ParticleEmitters = {
	icon = "icon16/asterisk_yellow.png",
	count = function(ply)
		return plyEmitterCount:get(ply)
	end,
}
...
return function(instance)
	...
	local particleem_methods, pewrap, peunwrap =
		instance.Types.ParticleEmitter.Methods,
		instance.Types.ParticleEmitter.Wrap,
		instance.Types.ParticleEmitter.Unwrap
	local particle_methods, pwrap, punwrap =
		instance.Types.Particle.Methods, instance.Types.Particle.Wrap, instance.Types.Particle.Unwrap
	local mat_meta, mwrap, munwrap =
		instance.Types.LockedMaterial, instance.Types.LockedMaterial.Wrap, instance.Types.LockedMaterial.Unwrap
	...
	function particleem_methods:add(
		material,
		position,
		startSize,
		endSize,
		...
		self = peunwrap(self)
		if not emitters[self] then
			SF.Throw("Tried to use invalid emitter!", 2)
		end
		...
		if dieTime < 0 or dieTime > 60 then
			SF.Throw("Die time must be between 0 and 60", 2)
		end

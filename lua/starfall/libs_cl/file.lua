...
registerprivilege(
	"file.read",
	"Read files",
	"Allows the user to read files from data/sf_filedata directory",
	{ client = { default = 1 } }
)
registerprivilege(
	"file.write",
	"Write files",
	"Allows the user to write files to data/sf_filedata directory",
	{ client = { default = 1 } }
)
registerprivilege(
	"file.writeTemp",
	"Write temporary files",
	"Allows the user to write temp files to data/sf_filedatatemp directory",
	{ client = {} }
)
registerprivilege(
	"file.exists",
	"File existence check",
	"Allows the user to determine whether a file in data/sf_filedata exists",
	{ client = { default = 1 } }
)
registerprivilege(
	"file.existsInGame",
	"File existence check",
	"Allows the user to determine whether a file in game dir exists",
	{ client = { default = 1 } }
)
registerprivilege(
	"file.isDir",
	"Directory check",
	"Allows the user to determine whether a file in data/sf_filedata is a directory",
	{ client = { default = 1 } }
)
registerprivilege(
	"file.find",
	"File find",
	"Allows the user to see what files are in data/sf_filedata",
	{ client = { default = 1 } }
)
registerprivilege(
	"file.findInGame",
	"File find in garrysmod",
	"Allows the user to see what files are in garrysmod",
	{ client = { default = 1 } }
)
registerprivilege(
	"file.open",
	"Get a file object",
	"Allows the user to use a file object",
	{ client = { default = 1 } }
)
registerprivilege(
	"file.time",
	"Get time modified",
	"Allows the user to see the last time a file was modified",
	{ client = { default = 1 } }
)
...
do
		...
		if ok then
			if self.entries[path] then
				...
				if file.Exists(path, "DATA") then
					SF.Throw("The existing file is currently locked!", 3)
				end
			end
			...
			if not f then
				SF.Throw("Couldn't open file for writing!", 3)
			end
			f:Write(data)
			f:Close()
			return "data/" .. path
	...
	function TempFileCache:CleanPly(check)
		-- Sort by time
		table.sort(check.plyentries, function(a, b)
			return a.time > b.time
		end)

		while
			(check.plycount >= cv_temp_maxfiles:GetInt() or check.plysize >= cv_temp_maxusersize:GetFloat() * 1e6)
			and #check.plyentries > 0
		do
	...
	function TempFileCache:CleanAll(check)
		...
		-- Sort by time
		table.sort(connectedplys, function(a, b)
			return a.time > b.time
		end)
		table.sort(disconnectedplys, function(a, b)
			return a.time > b.time
		end)
...
return function(instance)
	...
	local file_methods, file_meta, wrap, unwrap =
		instance.Types.File.Methods, instance.Types.File, instance.Types.File.Wrap, instance.Types.File.Unwrap
	...
	function file_library.readInGame(path)
		if instance.player ~= LocalPlayer() then
			SF.Throw("Only chip owner can read game files")
		end
		checkluatype(path, TYPE_STRING)
	...
	function file_library.asyncRead(path, callback)
		...
		if concurrentreads == cv_max_concurrent_reads:GetInt() then
			SF.Throw("Reading too many files asynchronously!", 2)
		end
	...
	local function checkExtension(filename)
		if not allowedExtensions[string.GetExtensionFromFilename(filename)] then
			SF.Throw("Invalid file extension!", 3)
		end
	end
	...
	function file_library.write(path, data)
		...
		if not f then
			SF.Throw("Couldn't open file for writing.", 2)
			return
		end
		f:Write(data)
		f:Close()
	...
	function file_library.readTemp(filename)
		...
		if #filename > 128 then
			SF.Throw("Filename is too long!", 2)
		end
	...
	function file_library.writeTemp(filename, data)
		...
		if tempfilewrites >= cv_temp_maxfiles:GetInt() then
			SF.Throw("Exceeded max number of files allowed to write!", 2)
		end
	...
	function file_library.find(path, sorting)
		...
		checkluatype(path, TYPE_STRING)
		if sorting ~= nil then
			checkluatype(sorting, TYPE_STRING)
		end

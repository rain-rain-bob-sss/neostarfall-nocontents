...
registerprivilege(
	"material.create",
	"Create material",
	"Allows users to create a new custom material.",
	{ client = {} }
)
registerprivilege(
	"material.imagecreate",
	"Create material from image",
	"Allows users to create a new material from an image file.",
	{ client = {} }
)
registerprivilege(
	"material.urlcreate",
	"Create material from online image",
	"Allows users to create a new material from an online image.",
	{ client = {}, urlwhitelist = {} }
)
registerprivilege(
	"material.datacreate",
	"Create material from base64 image data",
	"Allows users to create a new material from base64 image data.",
	{ client = {} }
)
...
local material_bank = SF.ResourceHandler(
	"render_usermaterials",
	"user materials",
	40,
	"The max number of user created materials",
	function(shader, i)
...
local function checkkey(key)
	...
	if blacklisted_keys[string.lower(key)] then
		SF.Throw("Blocked material key: " .. key, 3)
	end
end
...
return function(instance)
	...
	local material_methods, material_meta, wrap, unwrap =
		instance.Types.Material.Methods,
		instance.Types.Material,
		instance.Types.Material.Wrap,
		instance.Types.Material.Unwrap
	local lmaterial_methods, lmaterial_meta, lwrap, lunwrap =
		instance.Types.LockedMaterial.Methods,
		instance.Types.LockedMaterial,
		instance.Types.LockedMaterial.Wrap,
		instance.Types.LockedMaterial.Unwrap
	...
	function material_library.load(path)
		...
		if string.GetExtensionFromFilename(path) then
			SF.Throw("The path cannot have an extension", 2)
		end
		checkpermission(instance, path, "material.load")
		local m = SF.CheckMaterial(path)
		if not m or m:IsError() then
			SF.Throw("This material doesn't exist or is blacklisted", 2)
		end
		return lwrap(m)
	end
	...
	function material_library.create(shader)
		...
		if not allowed_shaders[shader] then
			SF.Throw("Tried to use unsupported shader: " .. shader, 2)
		end
	...
	function material_library.createFromImage(path, params)
		...
		if ext ~= "jpg" and ext ~= "png" then
			SF.Throw("Expected a .jpg or .png file", 2)
		end
		...
		if
			not (
				...
				or (
					(string.sub(path, 1, 5) == "data/" or string.sub(path, 1, 12) == "data_static/")
					and file.Exists(path, "GAME")
				)
			)
		then
		...
		for s in string.gmatch(string.lower(params), "%S+") do
			if not image_params[s] then
				SF.Throw("Invalid parameter: " .. s, 2)
			end
	...
	function lmaterial_methods:destroy() end
	...
	function material_methods:setTextureURL(key, url, cb, done)
		...
		if done ~= nil then
			checkluatype(done, TYPE_FUNCTION)
		end
	...
	function material_methods:setTextureRenderTarget(key, name)
		...
		local rt = instance.data.render.rendertargets[name]
		if not rt then
			SF.Throw("Invalid rendertarget: " .. name, 2)
		end
